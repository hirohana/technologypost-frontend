...
const useUserArticlePost = () => {
    ...
    uploadTask.on(
      ...
      () => {
        ...
        getDownloadURL(uploadTask.snapshot.ref).then((downloadURL) => {
          const newMarkdownValue = markdownValue + `![image](${downloadURL})\n`;
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    swal({
      ...
      saveDraftDataToBackEnd();
    });
  };

  /**
   * 送信ボタンを押した際にバックエンドに下書きデータを送信する関数
   */
  const saveDraftDataToBackEnd = async () => {
    const category = onSetCategory();

    // 画像ファイルがなかった場合に、以下のif文の処理を行わない為の条件分岐
    let stringImages = '';
    let stringFileNames = '';
    if (images.length !== 0) {
      stringImages = images.reduce((prev, current) => (prev += `,${current}`));
      stringFileNames = fileNames.reduce(
        (prev, current) => (prev += `,${current}`)
      );
    }
    ...
    try {
      ...
      if (response.status === 200) {
        navigate(`/user/${trimUserName}/article_list`);
      } else if (response.status === 500) {
        sweetAlertOfError(
          `サーバー側のエラーにより下書きデータが保存されませんでした。\nエラー内容: ${await response.json()}`
        );
